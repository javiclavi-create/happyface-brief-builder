...
from pgvector.psycopg import register_vector  # makes vectors work with psycopg

# ----- Settings -----
OPENAI_API_KEY = os.environ.get("OPENAI_API_KEY")
DATABASE_URL = os.environ.get("DATABASE_URL")
FRONTEND_ORIGIN = os.environ.get("FRONTEND_ORIGIN", "*")
# Use the 1536-dim model to match your database schema
...
# ----- Prompts -----
PITCH_SYSTEM_PROMPT = (
    "You are Happy Face Ads’ writers-room engine. You pitch bold, absurd, against-the-grain, "
    "comedic ad concepts that are faceless-friendly but can scale to actors. Lean on juxtaposition, "
    "pattern breaks, deadpan, absurd escalation, and clear CTAs. Keep hooks ≤12 words."
)
...
REWRITE_SYSTEM_PROMPT = (
    "You turn an approved ad concept into a production-ready short-form ad script in Happy Face tone: "
    "disruptive, funny, big swings, faceless-friendly; clear CTAs; no banned claims."
)
...
def ingest_text(req: IngestText):
    ...
    with get_conn() as conn, conn.transaction():
        ...
        for i, (chunk, v) in enumerate(zip(chunks, vecs)):
            conn.execute(
                """
                insert into embeddings(doc_id, chunk_idx, text_chunk, embedding)
                values (%s,%s,%s,%s)
                """,
                (doc["id"], i, chunk, v),
            )
